plugins {
    `java-library`
    `maven-publish`
    alias(libs.plugins.kotlin.jvm)
    alias(libs.plugins.kotlin.serialization)
    alias(libs.plugins.kotlinx.kover)
}

group = "io.skjaere"

repositories {
    mavenCentral()
    maven("https://jitpack.io")
}

java {
    toolchain {
        languageVersion = JavaLanguageVersion.of(25)
    }
    withSourcesJar()
}

dependencies {
    api("com.github.skjaere:ktor-nntp-client:v0.2.0")
    api("com.github.skjaere:kotlin-compression-utils:v0.3.0")
    implementation(libs.kotlinx.coroutines.core)
    implementation(libs.kotlinx.serialization.json)
    implementation(libs.slf4j.api)

    testImplementation(libs.kotlin.test)
    testImplementation(libs.junit.jupiter)
    testImplementation(libs.kotlinx.coroutines.test)
    testImplementation(libs.testcontainers)
    testImplementation("io.skjaere.mocknntp:mock-nntp-server")
}

tasks.test {
    useJUnitPlatform()
    val libPath = System.getenv("RAPIDYENC_LIB_PATH")
        ?: "${System.getProperty("user.home")}/IdeaProjects/rapidyenc/build"
    systemProperty("jna.library.path", libPath)
    systemProperty("kotlinx.coroutines.test.default.timeout", "PT5M")
}

publishing {
    publications {
        create<MavenPublication>("maven") {
            from(components["java"])
        }
    }
}
